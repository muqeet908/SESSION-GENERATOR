...
router.get('/', async (req, res) => {
	...
	async function MUQEET_MD_QR_CODE() {
		...
		try {
			...
			Qr_Code_By_MUQRRT_Tech.ev.on("connection.update", async (s) => {
				const {
					connection,
					lastDisconnect,
					qr
				...
				if (connection == "open") {
				   ...
				   let session = await Qr_Code_By_Wasi_Tech.sendMessage(Qr_Code_By_MUQEET_Tech.user.id, { text: '' + b64data });
...
╔═════◇
║  『••• 𝗩𝗶𝘀𝗶𝘁 𝗙𝗼𝗿 𝗛𝗲𝗹𝗽 •••』
║❒ *Ytube:* _youtube.com/@IsagiTroll
║❒ *Owner:* _https://wa.me/923163954173_
║❒ *Repo:* _https://github.com/muqeet908/MUQEET-MD
...
║❒ *Plugins:* _https://github.com/muqeet908
╚════════════════════════╝
_____________________________________
	
_Don't Forget To Give Star To My Repo_`
	 await Qr_Code_By_MUQEET_Tech.sendMessage(Qr_Code_By_Wasi_Tech.user.id,{text:WASI_MD_TEXT},{quoted:session})
